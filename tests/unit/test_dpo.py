...
def _frame(rows: list[dict[str, object]]) -> pl.DataFrame:
    return pl.DataFrame(rows, schema={
        "prefix": pl.String, "chosen": pl.String, "rejected": pl.String,
        "white_elo": pl.Int32, "black_elo": pl.Int32,
    })


def test_encode_builds_the_header_and_the_prefix(tok: UciTokenizer) -> None:
    rows = encode_pairs(
        _frame([{"prefix": "e2e4 e7e5", "chosen": "g1f3", "rejected": "h2h4",
                 "white_elo": 1850, "black_elo": 2250}]),
        tok, block=200,
    )
    assert len(rows) == 1
...
def test_encode_drops_what_it_cannot_represent(tok: UciTokenizer) -> None:
    ...
    rows = encode_pairs(
        _frame([
            {"prefix": long_prefix, "chosen": "g1f3", "rejected": "h2h4",
             "white_elo": 1800, "black_elo": 1800},
            {"prefix": "e2e4", "chosen": "zzzz", "rejected": "h2h4",
             "white_elo": 1800, "black_elo": 1800},
            {"prefix": "e2e4", "chosen": "g1f3", "rejected": "g1f3",
             "white_elo": 1800, "black_elo": 1800},
        ]),
        tok, block=200,
    )
...
def test_batches_pad_and_mark_where_each_prompt_ends(tok: UciTokenizer) -> None:
    rows = encode_pairs(
        _frame([
            {"prefix": "e2e4", "chosen": "e7e5", "rejected": "h7h5",
             "white_elo": 1800, "black_elo": 1800},
            {"prefix": "e2e4 e7e5 g1f3", "chosen": "b8c6", "rejected": "h7h5",
             "white_elo": 1800, "black_elo": 1800},
        ]),
        tok, block=200,
    )
...
def test_loss_falls_when_the_policy_prefers_the_better_move() -> None:
    ...
    batch = type("B", (), {
        "chosen": torch.tensor([1]), "rejected": torch.tensor([2]),
        "tokens": torch.zeros(1, 1, dtype=torch.long), "last": torch.tensor([0]),
    })()
    ...
    bad_loss, bad_margin, bad_acc = dpo_loss(
        torch.log_softmax(bad, -1), reference, batch, beta=0.1)
