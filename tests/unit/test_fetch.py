"""Tests for rukh.data.fetch: query builder and dry-run plan."""
...
def test_build_query_has_filters_and_both_months() -> None:
    ...
    assert "CAST(split_part(TimeControl, '+', 1) AS INTEGER) >= 180" in sql
...
def test_plan_lists_output_paths_and_manifest(rukh_home: Path) -> None:
    ...
    assert fetch_plan.out_paths == [
        str(out_dir / "year=2025" / "month=01" / "games.parquet"),
        str(out_dir / "year=2025" / "month=02" / "games.parquet"),
    ]
    assert fetch_plan.manifest_path == str(out_dir / "manifest.json")
...
def test_run_writes_parquet_and_manifest_from_local_source(
    ...
    duckdb.sql(
        ...
        """
    ).write_parquet(source.as_posix())
    monkeypatch.setattr(fetch_module, "_source", lambda cfg, month: source.as_posix())

    cfg = FetchConfig(months=["2025-01"])
    fetch_plan = run(cfg, dry_run=False)

    out = Path(fetch_plan.out_paths[0])
    assert out.is_file()
    assert duckdb.sql(f"SELECT count(*) FROM '{out.as_posix()}'").fetchone()[0] == 1
    manifest = Manifest.model_validate_json(Path(fetch_plan.manifest_path).read_text("utf-8"))
    assert manifest.counts == {"2025-01": 1}
    assert manifest.files[0].path == fetch_plan.out_paths[0]
