...
Scripts referenced by the M2 lesson (`rukh-lab`, `curso/m2/10-labs-del-decoder`). Run them from the
repository root.

| Script | Needs | Lesson lab |
|---|---|---|
| `params.py` | nothing | 1 · contar los parámetros a mano |
| `causal_mask.py` | nothing | 2 · dibujar la máscara causal |
| `attention_export.py` | a trained checkpoint | 3 · los 96 mapas · `artifacts/web/attention.json` |
| `replay_export.py` | an MLflow run and its `step-*.pt` files | 4 · la repetición · `artifacts/web/training-replay.json` |
