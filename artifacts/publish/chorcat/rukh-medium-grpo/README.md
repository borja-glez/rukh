...
written in UCI. This is the `medium-v4-grpo-greedy` stage of [Rukh](https://github.com/borja-glez/rukh), a course
that builds a chess language model end to end: 115,120,128 parameters, a
vocabulary of 2030 fixed tokens and a context of
200 moves.

Play against it in the browser: [https://rukh.borjaglez.com/?stage=medium-grpo-fp16](https://rukh.borjaglez.com/?stage=medium-grpo-fp16) · read how it was built:
...
| Puzzles solved | 38.6 % |
| Estimated Elo | 1577 (95 % CI 1526-1632) |
...
|---|---|---|---|
| Legality without the mask, argmax | at least 99 % | 99.3 % | met |
| Estimated Elo | at least 1200 | 1577 (95 % CI 1526-1632) | met |
...
`model-int8.onnx` does not reach it: it picks a different move in 3.0 % of
positions, roughly one in 33. That is the file the WASM fallback loads, so a phone on the
...
- Measured in the [single results table](https://lab.rukh.borjaglez.com/proyecto/) as the stage `medium-v4-grpo-greedy`: every stage of the course, the same suite, the same day.
- Play it in the browser: [https://rukh.borjaglez.com/?stage=medium-grpo-fp16](https://rukh.borjaglez.com/?stage=medium-grpo-fp16).
- Bring it to the paths the configs read: `uv run rukh pull medium-v4-grpo`.
