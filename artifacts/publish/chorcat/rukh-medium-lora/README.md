...
`(16, 2, 8, 768)` and `lora_b` `(16, 2, 768, 8)`, which carry a LoRA adapter stacked over the
blocks. Merging an adapter into the weights and exporting the result would cost a whole copy of
the model per style; this way a style is 1.6 MB fed on every call, and changing style costs an
upload rather than a download.

Two things have to hold and both are measured, on the same 1000 validation positions:
...
not in the graph, so an adapter of any rank is correct here without matching two numbers by hand;
