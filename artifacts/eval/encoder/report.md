...
- Suite: `encoder`
- Checkpoint: `checkpoints/encoder-heads-moves-20260919-101904/best.pt`
- Weights SHA-256: `321baaf4a06bfb96e837b0b2ae2fb0a668115f27f892173fbcba87569f20b367`
- Parameters: 15,054,725
- Device: `cuda`
- Date: 2026-09-19
- MLflow run: 804ae7e551a246d5a98dbfbc963ac5c7
...
| Value vs `tanh(cp / 400)`, Spearman (headline) | 0.520 |
...
The 7,453 labelled rows are cut in two by `game_id`, never by position, the policy the held-out split itself uses, because two positions of the same game are one move apart. The `tune` half (3,793 rows, 2,455 games) chose the threshold `p >= 0.06631` by maximising F1 there; the `score` half (3,660 rows, 2,368 games) is where every number below is measured, and no `game_id` is in both halves.
...
|---|---|---:|---:|---:|---:|---:|---:|
| encoder | tuned, `p >= 0.06631` (chosen on `tune`) | 3660 | 144 | 488 | 11.7 % | 39.6 % | 18.0 % |
| encoder | fixed, `p >= 0.5` | 3660 | 144 | 0 | 0.0 % | 0.0 % | 0.0 % |
| heuristic | rule, no threshold to tune | 3660 | 144 | 2359 | 4.7 % | 77.1 % | 8.9 % |

The same threshold reaches an F1 of 17.1 % on the `tune` half it was chosen on. The distance between that and the `score` half above is what picking an operating point costs, and it is the reason the two halves are not the same rows.
...
|---|---:|---:|---:|---:|---:|---|
| encoder | 3660 | 144 | 3.9 % | 0.740 | 0.112 | 0.0020 to 0.3102 (mean 0.0353) |
...
|---|---:|---|---:|---:|
| encoder | 10000 | `tanh(cp / 400)` | 0.520 | 0.648 |
...
- the encoder is +9.2 F1 points from the baseline; docs/acceptance.md asks for at least +5
