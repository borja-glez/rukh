# Evaluation of `encoder-rank`

- Suite: `encoder`
- Checkpoint: `checkpoints/encoder-heads-rank-20260920-110936/step-4000.pt`
- Weights SHA-256: `400e4bca3e0d163b6429d0783bce91cf285148c8faac9c8ced5421b57960b306`
- Parameters: 15,054,725
- Device: `cuda`
- Date: 2026-09-20
- MLflow run: a63ed003660d41048e329a203d3e48bc

## Headline

| Metric | Value |
|---|---|
| Blunder F1, encoder (tuned, `p >= 0.1055`) | 17.7 % |
| Blunder F1, encoder (fixed, `p >= 0.5`) | 0.0 % |
| Blunder F1, material baseline | 8.9 % |
| Margin over the baseline | +8.9 points |
| Margin >= 5 points | yes |
| Blunder ROC AUC | 0.750 |
| Blunder average precision | 0.115 |
| Blunder base rate | 3.7 % |
| Value vs `tanh(cp / 400)`, Spearman (headline) | 0.640 |
| Value vs `tanh(cp / 400)`, Pearson | 0.662 |
| Value correlation >= 0.80 | no |
| Result accuracy | 49.6 % |
...
The 7,453 labelled rows are cut in two by `game_id`, never by position, the policy the held-out split itself uses, because two positions of the same game are one move apart. The `tune` half (3,793 rows, 2,455 games) chose the threshold `p >= 0.1055` by maximising F1 there; the `score` half (3,660 rows, 2,368 games) is where every number below is measured, and no `game_id` is in both halves.
...
|---|---|---:|---:|---:|---:|---:|---:|
| encoder | tuned, `p >= 0.1055` (chosen on `tune`) | 3660 | 144 | 228 | 14.5 % | 22.9 % | 17.7 % |
| encoder | fixed, `p >= 0.5` | 3660 | 144 | 0 | 0.0 % | 0.0 % | 0.0 % |
| heuristic | rule, no threshold to tune | 3660 | 144 | 2359 | 4.7 % | 77.1 % | 8.9 % |

The same threshold reaches an F1 of 18.5 % on the `tune` half it was chosen on. The distance between that and the `score` half above is what picking an operating point costs, and it is the reason the two halves are not the same rows.
...
|---|---:|---:|---:|---:|---:|---|
| encoder | 3660 | 144 | 3.9 % | 0.750 | 0.115 | 0.0010 to 0.2569 (mean 0.0387) |
...
|---|---:|---|---:|---:|
| encoder | 10000 | `tanh(cp / 400)` | 0.640 | 0.662 |
...
- the encoder is +8.9 F1 points from the baseline; docs/acceptance.md asks for at least +5
- the value head is correlated against `tanh(cp / 400)`, the bounded score it is trained on, and not against raw `cp`, where a forced mate is worth ±9 99x and a few rows would decide Pearson for the whole set; the docs/acceptance.md bar of 0.80 is read on Spearman
...
- the two blunder detectors do not see the same thing: the baseline is given the predecessor position and the move that was played, while the encoder is given only the resulting position and has to infer that something was thrown away. That is the comparison docs/acceptance.md asks for, but it is not a level playing field
- the blunder threshold 0.1055 was chosen on the 'tune' half (3,793 rows, 2,455 games) by maximising F1 there, and the reported numbers are measured on the 'score' half (3,660 rows, 2,368 games), which no threshold ever saw; the same rows at the fixed threshold 0.5 give an F1 of 0.0000 against 0.1774 tuned
