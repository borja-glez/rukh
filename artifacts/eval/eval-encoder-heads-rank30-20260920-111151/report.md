# Evaluation of `eval-encoder-heads-rank30-20260920-111151`

- Suite: `encoder`
- Checkpoint: `checkpoints/encoder-heads-rank30-20260920-111151/step-4000.pt`
- Weights SHA-256: `0d9f894c87a0c8d497192e05c64fe0ece1f80f00256405dc3f23ed05286a800e`
- Parameters: 15,054,725
- Device: `cuda`
- Date: 2026-09-20
- MLflow run: 4a6b883d27b9471cb8e6e5d2eb43023c

## Headline

| Metric | Value |
|---|---|
| Blunder F1, encoder (tuned, `p >= 0.1053`) | 11.9 % |
| Blunder F1, encoder (fixed, `p >= 0.5`) | 0.0 % |
| Blunder F1, material baseline | 8.9 % |
| Margin over the baseline | +3.0 points |
| Margin >= 5 points | no |
| Blunder ROC AUC | 0.731 |
| Blunder average precision | 0.107 |
| Blunder base rate | 3.7 % |
| Value vs `tanh(cp / 400)`, Spearman (headline) | 0.660 |
| Value vs `tanh(cp / 400)`, Pearson | 0.611 |
...
The 7,453 labelled rows are cut in two by `game_id`, never by position, the policy the held-out split itself uses, because two positions of the same game are one move apart. The `tune` half (3,793 rows, 2,455 games) chose the threshold `p >= 0.1053` by maximising F1 there; the `score` half (3,660 rows, 2,368 games) is where every number below is measured, and no `game_id` is in both halves.
...
|---|---|---:|---:|---:|---:|---:|---:|
| encoder | tuned, `p >= 0.1053` (chosen on `tune`) | 3660 | 144 | 176 | 10.8 % | 13.2 % | 11.9 % |
| encoder | fixed, `p >= 0.5` | 3660 | 144 | 0 | 0.0 % | 0.0 % | 0.0 % |
| heuristic | rule, no threshold to tune | 3660 | 144 | 2359 | 4.7 % | 77.1 % | 8.9 % |

The same threshold reaches an F1 of 18.3 % on the `tune` half it was chosen on. The distance between that and the `score` half above is what picking an operating point costs, and it is the reason the two halves are not the same rows.
...
|---|---:|---:|---:|---:|---:|---|
| encoder | 3660 | 144 | 3.9 % | 0.731 | 0.107 | 0.0012 to 0.2655 (mean 0.0366) |
...
|---|---:|---|---:|---:|
| encoder | 10000 | `tanh(cp / 400)` | 0.660 | 0.611 |
...
- the encoder is +3.0 F1 points from the baseline; docs/acceptance.md asks for at least +5
...
- the blunder threshold 0.1053 was chosen on the 'tune' half (3,793 rows, 2,455 games) by maximising F1 there, and the reported numbers are measured on the 'score' half (3,660 rows, 2,368 games), which no threshold ever saw; the same rows at the fixed threshold 0.5 give an F1 of 0.0000 against 0.1188 tuned
