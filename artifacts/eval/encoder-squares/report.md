# Evaluation of `encoder-squares`

- Suite: `encoder`
- Checkpoint: `checkpoints/encoder-heads-20260919-100532/best.pt`
- Weights SHA-256: `ba252ec3bcbd99072d1c268f9a51eb1982b89774a26fef7ef2e529c9dac0d9da`
- Parameters: 14,260,997
- Device: `cuda`
- Date: 2026-09-19
- MLflow run: bcc27387a375412a98fec02193abf144
...
| Value vs `tanh(cp / 400)`, Spearman (headline) | 0.422 |
...
The 7,453 labelled rows are cut in two by `game_id`, never by position, the policy the held-out split itself uses, because two positions of the same game are one move apart. The `tune` half (3,793 rows, 2,455 games) chose the threshold `p >= 0.04459` by maximising F1 there; the `score` half (3,660 rows, 2,368 games) is where every number below is measured, and no `game_id` is in both halves.
...
|---|---|---:|---:|---:|---:|---:|---:|
| encoder | tuned, `p >= 0.04459` (chosen on `tune`) | 3660 | 144 | 596 | 9.1 % | 37.5 % | 14.6 % |
| encoder | fixed, `p >= 0.5` | 3660 | 144 | 0 | 0.0 % | 0.0 % | 0.0 % |
| heuristic | rule, no threshold to tune | 3660 | 144 | 2359 | 4.7 % | 77.1 % | 8.9 % |

The same threshold reaches an F1 of 13.2 % on the `tune` half it was chosen on. The distance between that and the `score` half above is what picking an operating point costs, and it is the reason the two halves are not the same rows.
...
|---|---:|---:|---:|---:|---:|---|
| encoder | 3660 | 144 | 3.9 % | 0.696 | 0.091 | 0.0022 to 0.1930 (mean 0.0287) |
...
|---|---:|---|---:|---:|
| encoder | 10000 | `tanh(cp / 400)` | 0.422 | 0.688 |
...
- the encoder is +5.7 F1 points from the baseline; docs/acceptance.md asks for at least +5
...
- the blunder threshold 0.04459 was chosen on the 'tune' half (3,793 rows, 2,455 games) by maximising F1 there, and the reported numbers are measured on the 'score' half (3,660 rows, 2,368 games), which no threshold ever saw; the same rows at the fixed threshold 0.5 give an F1 of 0.0000 against 0.1459 tuned
