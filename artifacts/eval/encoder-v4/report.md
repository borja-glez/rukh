...
| Metric | Value |
|---|---|
| Blunder F1, encoder (tuned, `p >= 0.09617`) | 18.6 % |
| Blunder F1, encoder (fixed, `p >= 0.5`) | 0.0 % |
| Blunder F1, material baseline | 8.9 % |
| Margin over the baseline | +9.7 points |
| Margin >= 5 points | yes |
| Blunder ROC AUC | 0.761 |
| Blunder average precision | 0.117 |
| Blunder base rate | 3.7 % |
| Value vs `tanh(cp / 400)`, Spearman (headline) | 0.666 |
| Value vs `tanh(cp / 400)`, Pearson | 0.726 |
...
The 7,453 labelled rows are cut in two by `game_id`, never by position, the policy the held-out split itself uses, because two positions of the same game are one move apart. The `tune` half (3,793 rows, 2,455 games) chose the threshold `p >= 0.09617` by maximising F1 there; the `score` half (3,660 rows, 2,368 games) is where every number below is measured, and no `game_id` is in both halves.
...
|---|---|---:|---:|---:|---:|---:|---:|
| encoder | tuned, `p >= 0.09617` (chosen on `tune`) | 3660 | 144 | 298 | 13.8 % | 28.5 % | 18.6 % |
| encoder | fixed, `p >= 0.5` | 3660 | 144 | 0 | 0.0 % | 0.0 % | 0.0 % |
| heuristic | rule, no threshold to tune | 3660 | 144 | 2359 | 4.7 % | 77.1 % | 8.9 % |

The same threshold reaches an F1 of 19.6 % on the `tune` half it was chosen on. The distance between that and the `score` half above is what picking an operating point costs, and it is the reason the two halves are not the same rows.
...
|---|---:|---:|---:|---:|---:|---|
| encoder | 3660 | 144 | 3.9 % | 0.761 | 0.117 | 0.0015 to 0.3212 (mean 0.0384) |
...
|---|---:|---|---:|---:|
| encoder | 10000 | `tanh(cp / 400)` | 0.666 | 0.726 |
...
- the encoder is +9.7 F1 points from the baseline; docs/acceptance.md asks for at least +5
